...
@Injectable()
export class RateLimiterUIDGuard implements CanActivate {
	...
	constructor(@Inject('RATE_LIMITER_OPTIONS_UID') private options: RateLimiterOptions, @Inject('Reflector') private readonly reflector: Reflector) {}
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		let consumeKey = request.headers['x-jk-uid']
		if(!consumeKey){
			return true
		}
